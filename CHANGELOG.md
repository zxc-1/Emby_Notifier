...
### 新增
- Web 管理台（`/` 与 `/admin`）：移动端玻璃态 UI、运行概述与最近入库展示。
- 最近入库增强：电影/剧集/18+ 药丸标签、海报展示、支持筛选（全部/非成人/成人），默认保留最新 30 条并持久化到 `/data/recent_entries.json`。
- 外观设置持久化：主题（默认跟随系统）、背景 URL/上传、随机背景（`https://api.rls.ovh/自适应`）等写入 `/data/ui_prefs.json`。
- Forward 联动持久化：MediaHelp 登录态落盘到 `/data/forward_mediahelp_state.json`，重启后自动恢复。
- 配置参数持久化：管理台「配置参数」保存到 `/data/.env.local`，升级/重启后自动加载，无需重复配置。
- 升级/热更新指引：提供「保留 `/data`」的更新方式（`pull`/`--build`），避免误用 `down -v` 造成配置丢失。

### 修复
- 修复「Forward 联动」开关被其它 Tab 保存误覆盖（导致重启/重进后自动关闭）的持久化问题；现仅在该开关字段出现在表单提交时才更新。
- 修复 Telegram 发送失败导致管理台「剧集聚合」无法回写的问题（不再抛异常阻断后续逻辑，管理台聚合回写正常执行）。
- 修复管理台轮询刷新导致的闪烁（仅在数据变化时更新列表）。
- 修复亮色模式下玻璃层级/边框对比度不足的问题。
- 18+ 外链详情链接：优先使用推送中“外链：详情”对应 URL（不再回退到 Emby 详情页）。
- 海报加载稳定性：管理台优先取 poster（含同目录 poster），并缓存到 `/data/poster_cache/`。
