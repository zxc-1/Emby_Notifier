...
- [4. docker-compose 部署](#4-docker-compose-部署)
  - [4.1 推荐 docker-compose.yaml](#41-推荐-docker-composeyaml)
- [5. Web 管理台（/ 与 /admin）](#5-web-管理台-与-admin)
  - [5.1 登录与权限](#51-登录与权限)
  - [5.2 忘记管理员密码如何恢复](#52-忘记管理员密码如何恢复)
  - [5.3 最近入库与示例](#53-最近入库与示例)
- [6. 数据持久化与从零重测](#6-数据持久化与从零重测)
  - [6.1 配置与登录状态检查](#61-配置与登录状态检查)
- [7. 配置项说明（环境变量）](#7-配置项说明环境变量)
  - [7.1 Telegram 设置](#71-telegram-设置)
  - [7.2 Emby 相关](#72-emby-相关)
  - [7.3 TMDB 相关（可选）](#73-tmdb-相关可选)
  - [7.4 mediainfo 等待策略](#74-mediainfo-等待策略)
  - [7.5 Webhook 安全](#75-webhook-安全)
  - [7.6 通知队列与重试（可选）](#76-通知队列与重试可选)
  - [7.7 Forward / MediaHelp 订阅外挂（可选）](#77-forward-mediahelp-订阅外挂可选)
- [8. 启动与验证](#8-启动与验证)
- [9. Emby Webhook 配置](#9-emby-webhook-配置)
- [10. 常见问题](#10-常见问题)
- [11. 健康检查与运行指标](#11-健康检查与运行指标)
  - [11.1 `/health` 接口](#111-health-接口)
  - [11.2 指标含义说明](#112-指标含义说明)
  - [11.3 典型排查思路](#113-典型排查思路)
---
...
管理台入口：`http://IP:端口/`（推荐）或 `http://IP:端口/admin`（兼容）。用于查看运行状态与最近入库，并在登录后配置推送/联动与外观。

包含：
- **运行概述**：队列长度、Worker、近 10 分钟推送等指标
- **最近入库**：电影/剧集/18+（含剧集聚合显示），支持筛选
- **配置/外观/Forward**：顶部导航与右上角齿轮（需登录才可保存）
